use std::{mem::{MaybeUninit, ManuallyDrop}, ops::{DerefMut, Deref}, hash::Hash};
...
impl<T, const MAX: usize> SuperSmallVec<T, MAX>{
    ...
    pub fn push(&mut self, data: T){
        if (self.size as usize) < MAX{
            self.size = self.size.checked_add(1).expect("Cannot hold more than 256 values in SmallSlice");
...
impl<T: std::fmt::Debug, const MAX: usize> std::fmt::Debug for SuperSmallVec<T, MAX>{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SmallSlice").field("data", &self.deref()).finish()
    }
}
